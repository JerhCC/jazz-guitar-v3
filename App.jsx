          ...
          {tab === 'practice' && (
            <div className="space-y-5">
              ...
              <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
                <p className="text-sm text-slate-400 mb-4">Select chords to practice. Use the metronome for timing.</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {Object.keys(CHORDS).filter((c) => showBarre || isOpen(c)).map((c) => (
                    <button key={c} onClick={() => playChord(CHORDS[c].frets)} className="bg-slate-700 hover:bg-cyan-600 px-3 py-2 rounded-lg text-sm font-semibold transition">
                      {c}
                    </button>
                  ))}
                </div>
